    ...
    def change_component_list(self, new_list):
        ...
        if self.listOfComponents is not None and not self.force_cache_all:
            if [c.hook_name for c in new_list] != [
                ...
                self._compute_cache()  # only recompute when the list changed and when the cache is partial
        self.listOfComponents = new_list
...
# TODO: So far this only supports patching experiments where you patch one component at a time
@define
class FastActivationStore(BaseActivationStore):
    """Stores the activations of a model for a given dataset (the patched dataset), and create hooks to patch the activations of a given component (head, layer, etc)."""

    comp_metric: CompMetric = field(kw_only=True)
    proba_edge: float = field(kw_only=True, default=1.)
    ...
    source_ids: List[Int] = field(init=False)
    target_ids: List[Int] = field(init=False)
    ...
    weights: Dict[str, List[float]] = field(init=False)
        ...
        for i in tqdm.tqdm(range(0, len(self.target_ids), batch_size)):
            ...
            for c in self.listOfComponents:
                ...
                if c.head >= 0:
                    ...
                    a_source = self.activation_cache[c.hook_name][source_idx,source_position]
                    a_target = self.activation_cache[c.hook_name][target_idx,target_position]
                    grad_target = self.grad_cache[c.hook_name][target_idx,target_position]
    ...
    def _create_ids(self):
        self.source_ids = []
        self.target_ids = []
        for target_id in range(len(self.dataset)):
            for source_id, _ in enumerate(self.dataset):
    ...
    def get_weights(self: "FastActivationStore", component_name: str):
        return self.weights[component_name]
